...
function printlist(l, type) {
	if (l.demo) {
		body.innerHTML += `<li><div class="card"><div class="card_body"><div class="list"><a href='${l.url}' target="_blank">${all_types[l.type]} #${type}: ${l.title}</a></div><a href='${l.demo}' class="demo-url" target="_blank">&#9757; See project</a></div><div class="timedate">${l.date}</div></div></li>`
	} else {
		body.innerHTML += `<li><div class="card"><div class="card_body"><div class="list"><a href='${l.url}' target="_blank">${all_types[l.type]} #${type}: ${l.title}</a></div></div><div class="timedate">${l.date}</div></div></li>`
	}
}
...
function all(data) {
	...
	data.forEach(d => {
		body.innerHTML +=
			'<br><div class="wrap"><i></i><span class="year">' +
			d.year +
			'</span><span class="projectperyear">Done ' +
			d.data.length +
			" projects this year</span></div><br><br>"
		d.data.forEach(l => {
			totalProjects++
